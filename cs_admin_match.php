...
if( ! function_exists('cs_admin_match') ) {//make it pluggable
	function cs_admin_match()
	{
		...
		if ( $errflag==0 and $action == "savenew" ) {
		  $sql = "insert into ". $cs_match ." values (0,'V'," . $_POST['spieltag'] . "," . $_POST['team1'] . "," . $_POST['team2'] . ",'" . $_POST['location'] . "','" . $_POST['matchtime'] . "',-1,-1,-1,-1,-1);";
		...
		if ( $errflag==0 and $action == "update" ) {
		  $sql = "update ".$cs_match." set tid1=" . $_POST['team1'] . ", tid2=" . $_POST['team2'] . ",location='" . $_POST['location'] . "',matchtime='" . $_POST['matchtime'] . "',spieltag=" . $_POST['spieltag'] . " where mid=".$_POST['mid'].";";
		  $results = $wpdb->query($sql);
		  if ( $results == 1 )
		...
		if ( get_option('cs_modus') == 2 )
		  $out .= '<th scope="col" style="width:10px;text-align: center">'.__('Spieltag',"wpcs").'</th>'."\n";	   
		...
		if ( get_option('cs_modus') == 2 )
		$out .= "<td style='text-align:center'>".($res->spieltag==-1?"-":$res->spieltag)."</td>";
