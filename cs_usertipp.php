...
// -----------------------------------------------------------------------------------
if( ! function_exists('show_UserTippForm') ) {//make it pluggable
			...
			if ( get_option('cs_modus') == 1 and (!isset($cs_col_group) or !$cs_col_group)) {
				if (get_option('cs_hovertable') == 1 and $res->round == "V") {
					...
					if (file_exists(  get_stylesheet_directory() . '/wp-championship/cs_groupstats.php' )){
						$hlink = get_stylesheet_directory_uri() . "/wp-championship/cs_groupstats.php"."?groupid=".($res->round == "V" ? $res->groupid : $res->mid);
					} else {
			...
			if (!$cs_col_match) {
				...
				if (get_option('cs_hovertable') == 1) { 
					...
					if (file_exists(  get_stylesheet_directory() . '/wp-championship/cs_matchstats.php' )){
						$hlink = get_stylesheet_directory_uri() . "/wp-championship/cs_matchstats.php"."?teamid=".$res->tid1;
					} else {
						$hlink = plugins_url('cs_matchstats.php',__FILE__)."?teamid=".$res->tid1;
					}
					
					$out .= "<a href='$hlink' id='$hid'  >".($res->round=='V' ? $res->team1:cs_team2text($res->team1)) . "</a> - ";
				} else
					$out .= ($res->round=='V' ? $res->team1:cs_team2text($res->team1)) . " - ";
				...
				if (get_option('cs_hovertable') == 1) {
					...
					if (file_exists(  get_stylesheet_directory() . '/wp-championship/cs_matchstats.php' )){
						$hlink = get_stylesheet_directory_uri() . "/wp-championship/cs_matchstats.php"."?teamid=".$res->tid2;
					} else {
						$hlink = plugins_url('cs_matchstats.php',__FILE__)."?teamid=".$res->tid2;
					}
					
					$out .= "<a href='$hlink' id='$hid'  >".($res->round=='V' ? $res->team2:cs_team2text($res->team2)) . "</a></td>";
				} else
					$out .= ($res->round=='V' ? $res->team2:cs_team2text($res->team2)) . "</td>";
			}
			if (!$cs_col_icon2) {
