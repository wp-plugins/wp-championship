...
if( ! function_exists('cs_admin_users') ) {//make it pluggable
	function cs_admin_users()
	{
		...
		if ( $action == "savenew" or $action == "update" ) {
			...
			if ( $action == "savenew" ) {
				...
				if ($results->anz == 0) {
					...
					$sql = "insert into ". $cs_users ." values (". $_POST['user'] . "," . $_POST['isadmin'] . "," . $_POST['mailservice'] . "," .$_POST['mailreceipt'] . "," . $_POST['stellv'] . ",".$_POST['champtipp'].",'1900-01-01 00:00:00',-1,'".$_POST['tippgroup']."');";
			...
			// update users
			if (  $action == "update" ) {
				$sql = "update ".$cs_users." set admin=" . $_POST['isadmin'] . ", mailservice=" . $_POST['mailservice'] .", mailreceipt=" . $_POST['mailreceipt'] . ",stellvertreter=" . $_POST['stellv'] . ",champion=" . $_POST['champtipp'] . ", tippgroup='".$_POST['tippgroup']."' where userid=".$_POST['user'].";";
				$results = $wpdb->query($sql);
		...
		if ( $action == 'remove' ) {
			$sql= "delete from ".$cs_users." where userid=".$_GET['userid'].";";
			$results = $wpdb->query($sql);
			if ( $results >= 1 )
